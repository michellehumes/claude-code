      ...
      <header className="bg-gradient-to-br from-pink-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-900 dark:to-purple-950/20">
        <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 flex items-center gap-3 lg:hidden">
            <span className="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">Share:</span>
            <div className="flex gap-2">
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(frontmatter.title)}&url=${encodeURIComponent(postUrl)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-lg bg-gray-100 p-2 text-gray-600 transition-colors hover:bg-pink-100 hover:text-pink-600 dark:bg-gray-700 dark:text-gray-400"
              ...
              <a
                href={`https://pinterest.com/pin/create/button/?url=${encodeURIComponent(postUrl)}&description=${encodeURIComponent(frontmatter.title)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-lg bg-gray-100 p-2 text-gray-600 transition-colors hover:bg-pink-100 hover:text-pink-600 dark:bg-gray-700 dark:text-gray-400"
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="lg:grid lg:grid-cols-12 lg:gap-8">
          ...
          <div className="lg:col-span-8">
            ...
            {/* End-of-post CTA */}
            <div className="mt-12 rounded-xl border border-pink-200 bg-gradient-to-r from-pink-50 to-purple-50 p-6 text-center dark:border-gray-700 dark:from-gray-800 dark:to-gray-800">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                Enjoying this article?
              </h3>
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                Browse more expert beauty guides, product reviews, and tutorials.
              </p>
              <div className="mt-4 flex flex-wrap justify-center gap-3">
                <Link
                  href={`/category/${frontmatter.category}`}
                  className="rounded-full bg-gradient-to-r from-pink-500 to-purple-600 px-5 py-2 text-sm font-medium text-white transition-all hover:from-pink-600 hover:to-purple-700"
                >
                  More {categoryToName(frontmatter.category)} Articles
                </Link>
                <Link
                  href="/posts"
                  className="rounded-full border border-pink-300 bg-white px-5 py-2 text-sm font-medium text-pink-600 transition-all hover:bg-pink-50 dark:border-gray-600 dark:bg-gray-700 dark:text-pink-400"
                >
                  Browse All Articles
                </Link>
              </div>
